...
class SignAccSGD(Optimizer):
    ...
    def __init__(
        ...
        super(SignAccSGD, self).__init__(params, defaults)
    ...
    def step(self, closure: OptLossClosure = None) -> OptFloat:
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                # version 1
                # p.data.add_(torch.sign(d_p), alpha=-group['lr'])
                # p.data.mul_(zeta)
                # p.data.add_(buf, alpha=1.0 - zeta)
                # version 2
                p.data.add_(torch.sign(d_p), alpha=-group['lr'])
                p.data.mul_(zeta)
                p.data.add_(torch.sign(buf), alpha=1.0 - zeta)
