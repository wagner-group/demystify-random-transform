...
class FABAttack():
    ...
    def _get_predicted_label(self, x):
        with torch.no_grad():
            outputs = self.predict(x, mode='test')
    ...
    def attack_single_run_targeted(self, x, y=None, use_rand_start=False):
        ...
        output = self.predict(x, mode='test')
    ...
    def perturb(self, x, y):
        adv = x.clone()
        with torch.no_grad():
            acc = self.predict(x, mode='test').max(1)[1] == y
            ...
            if not self.targeted:
                for counter in range(self.n_restarts):
                    ...
                    if ind_to_fool.numel() != 0:
                        ...
                        acc_curr = self.predict(adv_curr, mode='test').max(1)[
                            1] == y_to_fool
                        if self.norm == 'Linf':
                            res = (
