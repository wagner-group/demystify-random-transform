...
class SquareAttack():
    ...
    def margin_and_loss(self, x, y):
        ...
        if num_gpu > bs:
            zeros = torch.zeros((num_gpu - bs, c, h, w), device=x.device)
            x = torch.cat([x, zeros], dim=0)

        logits = self.predict(x, mode=self.mode)[:bs]
    ...
    def perturb(self, x, y=None):
        ...
        if y is None:
            if not self.targeted:
                with torch.no_grad():
                    output = self.predict(x, mode=self.mode)
        ...
        if not self.targeted:
            acc = self.predict(x, mode=self.mode).max(1)[1] == y
        else:
            acc = self.predict(x, mode=self.mode).max(1)[1] != y
        ...
        for counter in range(self.n_restarts):
            ...
            if ind_to_fool.numel() != 0:
                ...
                output_curr = self.predict(adv_curr, mode=self.mode)
                if not self.targeted:
