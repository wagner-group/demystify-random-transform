from pdb import Pdb
from adv.attacks.pgd_attack import PGDAttack
import os
import sys
import time
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        for _ in range(self.eot_iter):
            with torch.enable_grad():
                ...
                logits = self.model(x_adv, mode='attack')
...
class APGDAttack_targeted():
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        if num_gpu > bs:
            # print(x_in.size(), y_in.size())
            # x = x.repeat(num_gpu, 1, 1, 1)[:num_gpu]
            # y = y.repeat(num_gpu)[:num_gpu]
            x_zeros = torch.zeros((num_gpu - bs, c, h, w),
                                  device=x.device, dtype=x.dtype)
            x = torch.cat([x, x_zeros], dim=0)
            y_zeros = torch.zeros(num_gpu - bs, device=y.device, dtype=y.dtype)
            y = torch.cat([y, y_zeros], dim=0)
            assert x.size(0) == num_gpu and y.size(0) == num_gpu
            # print(torch.cuda.device_count(), bs)
            # print(x_in.size(), y_in.size())
            # import pdb
            # pdb.set_trace()
        ...
        # import pdb
        # pdb.set_trace()
        return x_best[:bs], acc[:bs], loss_best[:bs], x_best_adv[:bs]
