'''
Code in this file implements Parametric Noise Injection (PNI) and is taken from 
...
class noise_Linear(nn.Linear):
    ...
    def __init__(self, in_features, out_features, bias=True, pni='layerwise',
        ...
        if self.pni is 'layerwise':
            ...
            self.alpha_w = nn.Parameter(torch.ones(
                self.out_features).view(-1, 1) * 0.25, requires_grad=True)
        elif self.pni is 'elementwise':
            self.alpha_w = nn.Parameter(torch.ones(
                self.weight.size()) * 0.25, requires_grad=True)
...
class noise_Conv2d(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1, bias=True, pni='layerwise',
                 w_noise=True):
        ...
        if self.pni is 'layerwise':
            ...
            self.alpha_w = nn.Parameter(torch.ones(
                self.out_channels).view(-1, 1, 1, 1) * 0.25, requires_grad=True)
        elif self.pni is 'elementwise':
            self.alpha_w = nn.Parameter(torch.ones(
                self.weight.size()) * 0.25, requires_grad=True)
    ...
    def forward(self, input):
        ...
        noise_weight = self.weight + self.alpha_w * noise * self.w_noise
        output = F.conv2d(
            input, noise_weight, self.bias, self.stride, self.padding,
