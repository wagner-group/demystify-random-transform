...
class RandWrapperV6(RandWrapper):
...
class Aggregater(nn.Module):
    ...
    def __init__(self, num_tf, num_classes, num_samples, num_hidden=64):
        ...
        self.self_attn = nn.MultiheadAttention(num_hidden * 2, 1, batch_first=False)
    ...
    def forward(self, x, num_samples=None):
        ...
        num_tiles = int(np.ceil((self.num_samples - 1) / (num_samples - 1))) - 1
        emb_params_tf = emb_params[1:].tile((num_tiles, 1, 1))[:self.num_samples - num_samples]
        emb_softmax_tf = emb_softmax[1:].tile((num_tiles, 1, 1))[:self.num_samples - num_samples]
        ...
        attn_weight = self.self_attn(emb, emb, emb, need_weights=True)[1]    # [bs, ns, ns]
        attn_weight = attn_weight.unsqueeze(-1)
        softmax = softmax.view(num_samples, 1, batch_size, -1)
        softmax_tf = softmax[1:].tile((num_tiles, 1, 1, 1))[:self.num_samples - num_samples]
