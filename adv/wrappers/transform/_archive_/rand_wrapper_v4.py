...
class RandWrapperV4(RandWrapper):
    ...
    def _forward(self, inputs, num_draws=None, params=None, rule=None,
        ...
        tf_params = torch.vstack([torch.zeros_like(tf_params[:batch_size]), tf_params])
...
class Aggregater(nn.Module):
    ...
    def __init__(self, num_tf, num_classes, num_samples, num_hidden=64):
        ...
        self.ntf_weight = nn.Parameter(torch.tensor(0.05), requires_grad=True)
    ...
    def forward(self, x, num_samples=None):
        ...
        emb_params_tf = emb_params[1:].tile((num_tiles - 1, 1, 1))[:self.num_samples - num_samples]
        emb_softmax_tf = emb_softmax[1:].tile((num_tiles - 1, 1, 1))[:self.num_samples - num_samples]
        # Add noise to embeddings
        # noise_scale = 0.05 / np.sqrt(self.num_hidden)
        # noise_params_scale = emb_params_tf.detach().norm(2, -1, keepdim=True) * noise_scale
        # noise_softmax_scale = emb_softmax_tf.detach().norm(2, -1, keepdim=True) * noise_scale
        # emb_params_tf.add_(torch.randn_like(emb_params_tf, requires_grad=False) * noise_params_scale)
        # emb_softmax_tf.add_(torch.randn_like(emb_softmax_tf, requires_grad=False) * noise_softmax_scale)
        emb_params = torch.cat([emb_params, emb_params_tf], dim=0)
        ...
        weight = [self.ntf_weight, ] + [(1 - self.ntf_weight) / (self.num_samples - 1), ] * (self.num_samples - 1)
        weight = torch.vstack(weight).unsqueeze(0)    # shape: [1, num_draw, 1]
