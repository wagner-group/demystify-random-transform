...
class RandWrapperV7(RandWrapper):
...
class Aggregater(nn.Module):
    ...
    def forward(self, x, num_samples=None):
        ...
        # EDIT v13
        emb = torch.cat([emb_softmax, emb_params], dim=-1).view(self.num_samples * batch_size, -1)
        weight = self.fc(emb).view(self.num_samples, batch_size, 1)  # [ns, bs, 1]
        weight = F.softmax(weight, dim=0)
        softmax = softmax.view(num_samples, batch_size, -1)
        softmax_tf = softmax[1:].tile((num_tiles - 1, 1, 1))[:self.num_samples - num_samples]
        softmax = torch.cat([softmax, softmax_tf], dim=0)
        out = (weight * softmax).sum(0)
