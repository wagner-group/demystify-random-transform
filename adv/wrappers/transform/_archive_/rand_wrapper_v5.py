...
class RandWrapperV5(RandWrapper):
    ...
    def _forward(self, inputs, num_draws=None, params=None, rule=None,
        ...
        x = inputs.repeat(num_draws, 1, 1, 1) if repeat else inputs
...
class Aggregater(nn.Module):
    ...
    def forward(self, x, num_samples=None):
        ...
        tf_params, softmax = x
        batch_size = softmax.size(0) // (num_samples + 1)
        # assert softmax.size(0) == (num_samples + 1) * batch_size
        emb_params = self.tf_params_head(tf_params)
        emb_softmax = self.outputs_head(softmax)
        emb = self.combine_head(torch.cat([emb_softmax, emb_params], dim=-1))
        emb = emb.view(num_samples + 1, batch_size, -1)

        # v11
        num_tiles = int(np.ceil(self.num_samples / num_samples))
        emb_tf = emb[1:].tile((num_tiles - 1, 1, 1))[:self.num_samples - num_samples]
        emb_ntf = emb[0:1]
        emb_tf = torch.cat([emb[1:], emb_tf], dim=0)
        # assert emb_tf.size(0) == self.num_samples

        weight = (emb_ntf * emb_tf).sum(-1, keepdim=True)   # shape: [ns, nb, 1]
        softmax = softmax.view(num_samples + 1, batch_size, -1)[1:]
        softmax_tf = softmax.tile((num_tiles - 1, 1, 1))[:self.num_samples - num_samples]
        softmax = torch.cat([softmax, softmax_tf], dim=0)
        out = (F.softmax(weight, dim=0) * softmax).sum(0)
