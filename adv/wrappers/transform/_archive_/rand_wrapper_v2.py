...
from .. import EPS, get_bpda_transform, get_transform

# TF_BOOL_PARAM = ['boxblur', 'crop', 'erase', 'gaussblur', 'gray', 'gray1',
#                  'gray2', 'hflip', 'laplacian', 'medblur', 'motionblur',
#                  'sobel', 'solarize', 'vflip',
#                  # noise family
#                  'normal', 'pepper', 'poisson', 'salt', 'speckle', 'uniform']


class RandWrapperV2(RandWrapper):
    ...
    def _forward(self, inputs, num_draws=None, params=None, rule=None,
        ...
        x = inputs.repeat(num_draws - 1, 1, 1, 1) if repeat else inputs
        if x.size(0) != batch_size * (num_draws - 1):
...
class Aggregater(nn.Module):
    ...
    def __init__(self, num_tf, num_classes, num_samples, num_hidden=64):
        ...
        # self.norm_softmax = nn.LayerNorm(num_hidden)
        # self.norm_params = nn.LayerNorm(num_hidden)
        # self.norm_attn = nn.LayerNorm(num_hidden)
        # self.ff_block = nn.Sequential(
        #     nn.Linear(num_hidden, num_hidden),
        #     nn.ReLU(inplace=True),
        #     nn.Linear(num_hidden, num_classes),
        # )
        # self.norm_emb = nn.LayerNorm(2 * num_hidden)
    ...
    def forward(self, x, num_samples=None):
        ...
        emb_params = self.tf_params_head(tf_params).view(num_samples - 1, batch_size, -1)
        emb_softmax = self.outputs_head(softmax).view(num_samples, batch_size, -1)

        # v1-6
        # emb_params = torch.cat([torch.zeros_like(emb_params[:1]), emb_params], dim=0)

        # v7
        num_tiles = int(np.ceil((self.num_samples - 1) / (num_samples - 1)))
        emb_params = emb_params.tile((num_tiles, 1, 1))[:self.num_samples - 1]
        emb_params = torch.cat([torch.zeros_like(emb_params[:1]), emb_params], dim=0)
        emb_softmax_tf = emb_softmax[1:].tile((num_tiles - 1, 1, 1))[:self.num_samples - num_samples]
        emb_softmax = torch.cat([emb_softmax, emb_softmax_tf], dim=0)
        # assert emb_softmax.size(0) == emb_params.size(0) == self.num_samples

        # v8
        # num_tiles = int(np.ceil(self.num_samples / num_samples))
        # emb_params = torch.cat([torch.zeros_like(emb_params[:1]), emb_params], dim=0)
        # emb_params = emb_params.tile((num_tiles, 1, 1))[:self.num_samples]
        # emb_softmax = emb_softmax.tile((num_tiles, 1, 1))[:self.num_samples]

        # TODO: How to deal with variable length?

        # TODO: Q, K, V?
        # EDIT: version 1
        # NOTE: V is not really used here
        # weight = self.self_attn(emb_softmax, emb_params, emb_softmax, need_weights=True)[1]
        # weight = weight.unsqueeze(-1)
        # softmax = softmax.view(num_samples, batch_size, 1, -1).transpose(0, 1)
        # out = (weight * softmax).sum((1, 2)) / num_samples
        # EDIT: version 2
        # emb_softmax = self.norm_softmax(emb_softmax)
        # emb_params = self.norm_params(emb_params)
        # out = self.self_attn(emb_softmax, emb_params, emb_softmax, need_weights=True)[0]
        # out = (out + emb_softmax).mean(0)
        # out = self.ff_block(self.norm_attn(out))
        # EDIT: version 3
        # TODO: ntf_weight = 0.5
        # ntf_weight = 1 / self.num_samples
        # weight = torch.zeros((1, num_samples, 1), device=softmax.device)
        # weight[0, 0, 0] = ntf_weight
        # weight[0, 1:, 0] = (1 - ntf_weight) / (num_samples - 1)
        # softmax = softmax.view(num_samples, batch_size, 1, -1).transpose(0, 1)
        # out = (weight * softmax).sum((1, 2)) / num_samples
        # EDIT: version 4
        # emb = torch.cat([emb_softmax, emb_params], dim=-1)
        # emb = self.norm_emb(emb)
        # weight = self.self_attn(emb, emb, emb, need_weights=True)[1]    # [bs, ns]
        # ntf_weight = 1 / self.num_samples
        # weight[:, 0] = ntf_weight
        # weight[:, 1:] = (1 - ntf_weight) / (num_samples - 1)
        # weight = weight.unsqueeze(-1)
        # softmax = softmax.view(num_samples, batch_size, 1, -1).transpose(0, 1)
        # out = (weight * softmax).sum((1, 2)) / num_samples
        # EDIT: version 5
        # NOTE: normalize inside tf_params_head and outputs_head (v5), no norm or relu (v6)
        ...
        # out = ((attn_weight * softmax).sum(2) * weight).sum(1)
        # EDIT: version 7: duplicate tf samples
        attn_weight = self.self_attn(emb_softmax, emb_params, emb_softmax, need_weights=True)[1]    # [bs, ns, ns]
        attn_weight = attn_weight.unsqueeze(-1)
        softmax = softmax.view(num_samples, 1, batch_size, -1)
        softmax_tf = softmax[1:].tile((num_tiles - 1, 1, 1, 1))[:self.num_samples - num_samples]
        softmax = torch.cat([softmax, softmax_tf], dim=0).transpose(0, 2)
        out = (attn_weight * softmax).sum((1, 2)) / self.num_samples
        # EDIT: version 8: IGNORE
        # attn_weight = self.self_attn(emb_softmax, emb_params, emb_softmax, need_weights=True)[1]    # [bs, ns, ns]
        # attn_weight = attn_weight.unsqueeze(-1)
        # softmax = softmax.view(num_samples, 1, batch_size, -1)
        # softmax = softmax.tile((num_tiles, 1, 1, 1))[:self.num_samples].transpose(0, 2)
        # out = (attn_weight * softmax).sum((1, 2)) / self.num_samples
