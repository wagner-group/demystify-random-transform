...
class y_dequantize(nn.Module):
    ...
    def forward(self, image, factor=1.):
        return image * (self.y_table.to(image.device) * factor)
...
class c_dequantize(nn.Module):
    ...
    def forward(self, image, factor=1.):
        return image * (self.c_table.to(image.device) * factor)
...
class idct_8x8(nn.Module):
    ...
    def __init__(self):
        ...
        # self.alpha = nn.Parameter(torch.from_numpy(np.outer(
        #     alpha, alpha)).float(), requires_grad=False)
    ...
    def forward(self, image):
        ...
        result = 0.25 * \
            torch.tensordot(image, self.tensor.to(image.device), dims=2) + 128
...
class ycbcr_to_rgb_jpeg(nn.Module):
    ...
    def forward(self, image):
        result = torch.tensordot(image + self.shift.to(image.device),
                                 self.matrix.to(image.device), dims=1)
...
class decompress_jpeg(nn.Module):
    ...
    def forward(self, y, cb, cr, factor=1.):
