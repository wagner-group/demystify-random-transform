...
class rgb_to_ycbcr_jpeg(nn.Module):
    ...
    def forward(self, image):
        ...
        # print(image.device, self.matrix.device, self.shift.device)
        result = torch.tensordot(image, self.matrix.to(
            image.device), dims=1) + self.shift.to(image.device)
...
class dct_8x8(nn.Module):
    ...
    def __init__(self):
        ...
        #     np.outer(alpha, alpha) * 0.25).float(), requires_grad=False)
        self.register_buffer(
    ...
    def forward(self, image):
        image = image - 128
        result = self.scale.to(image.device) * torch.tensordot(
            image, self.tensor.to(image.device), dims=2)
        result.view(image.shape)
        return result
...
class y_quantize(nn.Module):
    ...
    def forward(self, image, factor=1.):
        image = image.float() / (self.y_table.to(image.device) * factor)
...
class c_quantize(nn.Module):
    ...
    def forward(self, image, factor=1.):
        image = image.float() / (self.c_table.to(image.device) * factor)
        image = self.rounding(image)
        return image
